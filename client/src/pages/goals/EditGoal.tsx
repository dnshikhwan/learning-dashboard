...
const EditGoal = () => {
  ...
  useEffect(() => {
    const fetchGoal = async () => {
      try {
        const response = await axiosConfig.get(`/goals/goal/${id}`);
        console.log(response.data);
        const goalData = response.data.details.data.goal[0] as IGoal;
        setDescription(goalData.description);
        setStatus(goalData.status);
        setTargetDate(format(new Date(goalData.target_date), "yyyy-MM-dd"));
      } catch (err) {
  ...
  const handleUpdateGoal = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axiosConfig.put(`/goals/${id}`, data);
